...
basepath = Path(ROOT) / "SaReMaRa_20260519_NeuralAnalysis/GoalProgressCohort"
...
for session in sessions: 
    ...
    cellpose_fpath = basepath / session / 'funcimg/suite2p/plane0/meanImg_chan1_seg.npy'
    ...
    wd_path = basepath / session / 'funcimg/rec2/suite2p/plane0/'
    wd = Path(wd_path)
    ...
    # Save dummy redcell.npy to match ROI number to cellpose results (produced only from chan1 meanImg; causes bug for opening stat.npy in s2p GUI)
    redcell = np.zeros((len(stat), 2), dtype=np.float32)  # col0: is_red, col1: red_prob
    np.save(wd / 'redcell.npy', redcell)
